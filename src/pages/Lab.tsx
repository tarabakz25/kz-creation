...
const imagesGlob = import.meta.glob<string>(
...
function getImage(path: string): string | null {
  ...
  return imagesGlob[normalized] ?? null;
}
      ...
      <main className="h-screen flex justify-between px-[5vw] py-[10vh] overflow-hidden relative">
        ...
        <div className="w-2/3 h-full overflow-y-auto flex flex-col items-end gap-8 pb-20 pr-2">
          {works.map((work, index) => (
            ...
            >
              <h2 className={`text-2xl tracking-wider font-futura_pt leading-none mb-2 ${selectedWork?.name === work.name ? 'text-white/30' : 'text-white'}`}>
                {work.name}
              </h2>
              <div className="flex gap-2">
                {work.tags.map((tag) => (
                  <span key={tag} className="text-xs font-avenir text-white/50 px-2 py-0.5">
                    #{tag}
                  </span>
                ))}
              </div>
        ...
        {selectedWork && (
          ...
          >
            ...
            >
              ...
              <div className={`flex flex-col justify-between w-full ${selectedWork.imageUrl ? 'md:w-1/2' : ''} text-white`}>
                <div>
                  <h3 className="text-3xl font-bold font-avenir mb-4">{selectedWork.name}</h3>
                  <div className="flex flex-wrap gap-1 mb-6">
                    {selectedWork.tags.map((tag) => (
                      <span key={tag} className="text-sm text-white/70 font-avenir px-2 py-1">#{tag}</span>
                    ))}
                  </div>
                </div>
                {selectedWork.url ? (
                  <a
                    ...
                    className="inline-flex items-center justify-center gap-2 font-avenir tracking-wider text-white px-6 py-3 hover:text-white/60 transition-colors mt-4 md:mt-0"
                  >
